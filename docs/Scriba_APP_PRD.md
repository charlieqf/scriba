...
### 功能说明  
- 支持第三方快捷登录  
- 允许用户选择角色（影响推荐模板，**注意：用户注册后不可自行更改，需通过“联系管理员”入口申请修改**）  
- **权限说明**：手机端用户仅拥有**查看和编辑生成报告**的权限；高级管理与控制功能由管理员在后台系统操作。  
- Face ID 可以作为快速登录方式  
- 支持设备级别的权限引导（麦克风、蓝牙、通知）  
- 提供 Onboarding 结束后的新手指引  
- 提供 **“联系管理员”** 入口（用于角色修改申请等）
...
- **性能**：冷启动 < 1.5 秒  
- **延迟**：录音启动延迟 < 50ms；AI 处理延迟 < 录音时长的 20%（例如 60 分钟录音 < 12 分钟）  
- **可靠性**：上传重试 3 次  
